...
Visualizer::Visualizer(
	...
	: menu_count_(0), reverse_menu_count_(0), settings_(settings), keyManager_(std::move(keyManager)) {

	if (!glfwInit()) {
		throw std::exception{};
	}

	window_ = glfwCreateWindow(std::get<int>(settings_->GetValue("visual", "window_width")),
							   std::get<int>(settings_->GetValue("visual", "window_height")),
							   std::get<std::string>(settings_->GetValue("visual", "window_name")).c_str(),
							   NULL,
							   NULL);
...
void Visualizer::ShowPlayer(int x, int y) {
	...
	GLuint TextureID;
	Textures::Texture::LoadTexture("../Texture/Mario/mario-idle.png", &TextureID, GL_REPEAT, GL_NEAREST, NULL);
	...
	glTexCoord2f(0, 0);	glVertex2d(0,
								y * field_pixel);
	glTexCoord2f(0, 1);	glVertex2d(0 + field_pixel,
								y * field_pixel);
	glTexCoord2f(1, 1);	glVertex2d(0 + field_pixel,
								y * field_pixel + field_pixel);
	glTexCoord2f(1, 0);	glVertex2d(0,
								y * field_pixel + field_pixel);
...
void Visualizer::PrintBlock(size_t x, size_t y, int type) {
	...
	GLuint TextureID;
	Textures::Texture::LoadTexture("../Texture/ground.png", &TextureID, GL_REPEAT, GL_NEAREST, NULL);
	...
	switch (type) {
		...
		case 3:glColor3f(0, 0, 1);
			break;
		case 4:glColor3f(0, 1, 1);
			break;
		case 5:glColor3f(0, 0, 1);
			break;
	}
	...
	glTexCoord2f(0, 0); glVertex2d(diff * field_pixel,
			   					y * field_pixel);
	glTexCoord2f(0, 1); glVertex2d(diff * field_pixel + field_pixel,
			   					y * field_pixel);
	glTexCoord2f(1, 1); glVertex2d(diff * field_pixel + field_pixel,
			   					y * field_pixel + field_pixel);
	glTexCoord2f(1, 0); glVertex2d(diff * field_pixel,
			   					y * field_pixel + field_pixel);
